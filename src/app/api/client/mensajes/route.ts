...
export async function POST(request: NextRequest) {
  try {
    const { caso_id, suscriptor_id, contenido, nombre } = await request.json();
    if (!caso_id || !suscriptor_id || !contenido?.trim()) {
    ...
    const { data, error } = await supabase
      .from("mensajes")
      .insert({ caso_id, autor_tipo: "cliente", autor_id: suscriptor_id, autor_nombre: nombre || "Cliente", contenido: contenido.trim() })
      .select()
      .single();
