  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-white text-xl font-bold">Vendedores</h1>
          <p className="text-beige/40 text-sm mt-1">Gestiona tu equipo comercial y sus comisiones</p>
        </div>
          ...
          <button
            ...
            className={`p-2.5 rounded-lg border transition-colors ${
              showSettings
                ...
                : "bg-white/5 border-white/10 text-beige/40 hover:text-white hover:border-white/20"
            }`}
            title="Configuración"
          >
      ...
      {/* ═══ SETTINGS PANEL ═══ */}
      {showSettings && (
        <div className="bg-white/5 border border-oro/20 rounded-xl overflow-hidden">
          {/* Settings header */}
          <div className="flex items-center justify-between px-5 py-3 border-b border-white/10 bg-oro/5">
            <div className="flex items-center gap-2">
              <Settings className="w-4 h-4 text-oro" />
              <span className="text-white text-sm font-bold">Configuración de ventas</span>
            </div>
            <button onClick={() => setShowSettings(false)} className="p-1 text-beige/40 hover:text-white transition-colors">
              <X className="w-4 h-4" />
            </button>
          </div>
          ...
          <div className="p-5 space-y-6">
            ...
            <div>
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-white text-sm font-bold">Comisiones</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  >
                    <option value="fijo" className="bg-jungle-dark">Monto fijo por cierre</option>
                    <option value="porcentaje" className="bg-jungle-dark">% del valor del plan</option>
                  </select>
                </div>
                {config.comision_tipo === "fijo" ? (
                  <div>
                    ...
                    <input
                      ...
                      step="5000"
                    />
                    <p className="text-beige/20 text-[10px] mt-1">Cada cierre le paga ${config.comision_fija.toLocaleString()} al vendedor</p>
                  </div>
                ) : (
                  <div>
                    ...
                    <input
                      ...
                      max="100"
                    />
                    <p className="text-beige/20 text-[10px] mt-1">Se calcula sobre el primer pago del cliente</p>
                  </div>
                )}
                <div>
                  ...
                  <select
                    ...
                    className={`${inputCls} appearance-none`}
                  >
                    <option value="quincenal" className="bg-jungle-dark">Quincenal</option>
                    <option value="mensual" className="bg-jungle-dark">Mensual</option>
                  </select>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-white/10 pt-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-white text-sm font-bold">Metas y bonificaciones</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                <div>
                  ...
                  <input
                    ...
                    min="0"
                  />
                  <p className="text-beige/20 text-[10px] mt-1">Cierres esperados por vendedor al mes</p>
                </div>
                <div>
                  ...
                  />
                  <p className="text-beige/20 text-[10px] mt-1">Al llegar a {config.bonificacion_meta} cierres</p>
                </div>
                <div>
                  ...
                  <input
                    ...
                    disabled={!config.bonificacion_activa}
                  />
                  <p className="text-beige/20 text-[10px] mt-1">Bono extra: ${config.bonificacion_monto.toLocaleString()}</p>
                </div>
              </div>
            </div>
            ...
            <div className="border-t border-white/10 pt-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-white text-sm font-bold">Reglas de cierre</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    min="1"
                  />
                  <p className="text-beige/20 text-[10px] mt-1">Después de {config.dias_para_cerrar} días el lead no cuenta como cierre del vendedor</p>
                </div>
                <div>
                  <label className={labelCls}>
                    <span className="flex items-center gap-2">
                  ...
                  <p className="text-beige/20 text-[10px] mt-2">
            ...
            <div className="border-t border-white/10 pt-5">
              <div className="flex items-center gap-2 mb-3">
                ...
                <h3 className="text-white text-sm font-bold">Links y mensajes</h3>
              </div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  />
                  <p className="text-beige/20 text-[10px] mt-1">El link será: {config.url_base}/r/CODIGO</p>
                </div>
                <div>
                  <label className={labelCls}>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-white/10">
              <button
                ...
                className="flex items-center gap-1.5 text-beige/40 hover:text-white text-xs transition-colors"
              >
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          { label: "Vendedores", value: totalVendedores, icon: BadgeDollarSign, color: "text-purple-400" },
          { label: "Leads totales", value: totalLeads, icon: Users, color: "text-blue-400" },
          { label: "Cierres", value: totalConvertidos, icon: TrendingUp, color: "text-green-400" },
      ...
      {/* Config summary bar */}
      <div className="flex items-center gap-4 text-[11px] text-beige/30 bg-white/[0.02] border border-white/5 rounded-lg px-4 py-2">
        ...
        <span>Meta: <span className="text-purple-400">{config.meta_mensual} cierres/mes</span></span>
        <span className="text-white/10">|</span>
        <span>Pago: <span className="text-blue-400">{config.pago_frecuencia}</span></span>
        {config.bonificacion_activa && (
          <>
            <span className="text-white/10">|</span>
            <span>Bono: <span className="text-green-400">${config.bonificacion_monto.toLocaleString()} al llegar a {config.bonificacion_meta}</span></span>
          </>
        )}
      ...
      {vendedores.length === 0 ? (
        <div className="bg-white/5 border border-white/10 rounded-xl p-12 text-center">
          <BadgeDollarSign className="w-10 h-10 text-beige/20 mx-auto mb-3" />
          <p className="text-beige/40 text-sm mb-4">No hay vendedores registrados</p>
          <Link href="/admin/vendedores/nuevo">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-white/10">
                  <th className="text-left text-beige/40 text-xs font-medium px-4 py-3">Vendedor</th>
                  <th className="text-left text-beige/40 text-xs font-medium px-4 py-3">Código</th>
                  <th className="text-center text-beige/40 text-xs font-medium px-4 py-3">Leads</th>
                  <th className="text-center text-beige/40 text-xs font-medium px-4 py-3">Nuevos</th>
                  <th className="text-center text-beige/40 text-xs font-medium px-4 py-3">Cierres</th>
                  <th className="text-center text-beige/40 text-xs font-medium px-4 py-3">Meta</th>
                  <th className="text-center text-beige/40 text-xs font-medium px-4 py-3">Tasa</th>
                  <th className="text-right text-beige/40 text-xs font-medium px-4 py-3">Comisión</th>
                  ...
                  <tr key={v.id} className="border-b border-white/5 hover:bg-white/[0.03] transition-colors">
                    <td className="px-4 py-3">
                      <Link href={`/admin/vendedores/${v.id}`} className="flex items-center gap-3 group">
                        <div className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-xs" style={{ backgroundColor: v.color }}>
                          {v.nombre.charAt(0)}
                        </div>
                        <div>
                          <p className="text-white text-sm font-medium group-hover:text-oro transition-colors">{v.nombre}</p>
                          <p className="text-beige/30 text-xs">{v.email}</p>
                        </div>
                      </Link>
                    </td>
                    <td className="px-4 py-3">
                      <code className="text-oro text-xs bg-oro/10 px-2 py-0.5 rounded">{v.vendedor_code || "—"}</code>
                    </td>
                    <td className="px-4 py-3 text-center text-beige/60">{v.total}</td>
                    <td className="px-4 py-3 text-center">
                      {v.nuevos > 0 ? (
                        <span className="text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded-full text-xs font-medium">{v.nuevos}</span>
                      ) : (
                        <span className="text-beige/20">0</span>
                      )}
                    </td>
                    <td className="px-4 py-3 text-center">
                      {v.convertidos > 0 ? (
                        <span className="text-green-400 font-bold">{v.convertidos}</span>
                      ) : (
                        <span className="text-beige/20">0</span>
                      )}
                    </td>
                        ...
                        <div className="w-12 h-1.5 bg-white/10 rounded-full overflow-hidden">
                          <div
                            className={`h-full rounded-full transition-all ${
                              v.convertidos >= config.meta_mensual ? "bg-green-400" : v.convertidos >= config.meta_mensual * 0.5 ? "bg-oro" : "bg-white/20"
                            }`}
                            style={{ width: `${Math.min((v.convertidos / Math.max(config.meta_mensual, 1)) * 100, 100)}%` }}
                          />
                        </div>
                        <span className="text-beige/30 text-[10px]">{v.convertidos}/{config.meta_mensual}</span>
                      </div>
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`text-xs font-medium ${v.tasa >= 30 ? "text-green-400" : v.tasa >= 15 ? "text-oro" : "text-beige/40"}`}>
                        {v.tasa}%
                      </span>
                    </td>
                    <td className="px-4 py-3 text-right">
                      <span className="text-oro font-bold text-sm">${v.comisionTotal.toLocaleString()}</span>
                      {v.cumplioMeta && <span className="text-green-400 text-[9px] block">+bono</span>}
                    </td>
                    <td className="px-4 py-3 text-center">
                      <span className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${
                        v.estado === "activo" ? "bg-green-500/15 text-green-400" : "bg-red-500/15 text-red-400"
                      }`}>
                        {v.estado}
                      </span>
                    </td>
